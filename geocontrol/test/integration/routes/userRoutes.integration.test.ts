...
describe("UserRoutes integration", () => {
  ...
  it("get all users: 500 generic error", async () => {
    (authService.processToken as jest.Mock).mockResolvedValue(undefined);
    ...
    expect(response.body.message).toMatch(/Generic error/);
  });

  it("create user: 201 success", async () => {
    const newUser = {
      username: "newuser",
      password: "securepass",
  ...
  it("delete user: 204 success", async () => {
    (authService.processToken as jest.Mock).mockResolvedValue(undefined);
    ...
    expect(userController.deleteUser).toHaveBeenCalledWith("newuser");
  });
  ...
  it("delete user: 500 generic error", async () => {
    (authService.processToken as jest.Mock).mockResolvedValue(undefined);
